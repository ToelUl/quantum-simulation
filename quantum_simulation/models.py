...
def kitaev_chain_bogoliubov_spectrum(k: Union[float, np.ndarray],
                                     hopping: float = 1.0,
                                     pairing_gap: float = 1.0,
                                     chemical_potential: float = 1.0,
                                     phi: float = 0.0
                                     ) -> tuple[Union[float, np.ndarray], Union[float, np.ndarray]]:
    """Calculates the Bogoliubov spectrum for the Kitaev chain model.
    This function computes the energy spectrum of the Kitaev chain, a
    prototypical model for a 1D p-wave superconductor that can host
    Majorana zero modes. The spectrum is derived from the model's parameters,
    including hopping amplitude, superconducting pairing gap, chemical potential,
    and an optional phase factor.
    ...
    Args:
        ...
        phi: An optional phase factor that can introduce complex pairing terms.
    ...
    return xy_chain_bogoliubov_spectrum(k=k, jx=4.0 * jx, jy=4.0 * jy, h=2.0 * h, phi=phi)
...
def kitaev_chain_ground_energy(lattice_length: int = 4,
                               hopping: float = 1.0,
                               pairing_gap: float = 1.0,
                               chemical_potential: float = 1.0,
                               phi: float = 0.0,
                               use_abc: bool = True) -> float:
    """Calculates the ground state energy of the Kitaev chain model.
